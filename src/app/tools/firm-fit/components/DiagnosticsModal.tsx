    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`${result.firm.name} breach details`}>
      <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-purple-500/30 bg-slate-950 p-5 shadow-2xl shadow-purple-950/30 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-purple-300">Diagnostics</p><h2 className="mt-2 text-2xl font-bold text-white">{result.firm.name} · {result.model.account_model}</h2></div>
        ...
        {result.status !== "PASSED" && <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Daily drawdown comparison</p><p className="mt-2 text-sm text-slate-200">Firm limit: <strong>{result.firm.rules.maxDailyDrawdown === null ? "Not supplied" : `${result.firm.rules.maxDailyDrawdown.toFixed(1)}%`}</strong></p><p className="mt-1 text-sm text-slate-200">Your max: <strong>{result.userMaxDailyDD.toFixed(1)}%</strong></p></div>
