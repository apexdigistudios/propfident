  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      {results.map((result) => {
        ...
        return (
          <article key={result.firm.id} className="rounded-2xl border border-purple-900/30 bg-slate-900/70 p-5 shadow-xl shadow-indigo-950/20 backdrop-blur-xl transition-all duration-300 hover:border-purple-500/40">
            ...
            <div className="mt-5 flex flex-col gap-2 sm:flex-row">
              <a href={result.firm.affiliateUrl} target="_blank" rel="noreferrer" className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-purple-400/30 bg-gradient-to-r from-purple-600 via-violet-600 to-indigo-600 px-3 py-2.5 text-center text-xs font-semibold text-white shadow-lg shadow-purple-600/25 transition-all hover:from-purple-500 hover:to-indigo-500">Get {result.firm.name} Account ({result.firm.discountPercentage}% Off) <ExternalLink className="h-3.5 w-3.5" /></a>
              <button type="button" onClick={() => onDetails(result)} className="rounded-xl border border-slate-700/60 bg-slate-800/60 px-3 py-2.5 text-xs font-bold text-slate-200 transition-all hover:border-purple-500/40 hover:bg-slate-800">View Breach Details</button>
            </div>
          </article>
        );
      })}
